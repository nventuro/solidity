...
function generate_bytecode_report() {
  ...
  (
    ...
    sed -i -e 's/rm -rf "\$TMPDIR"/cp "\$TMPDIR"\/report.txt \/tmp\/report.txt ; rm -rf "\$TMPDIR"/' \
      scripts/bytecodecompare/storebytecode.sh
    export SOLC_EMSCRIPTEN="On"
    scripts/bytecodecompare/storebytecode.sh >/dev/null 2>&1
  )
  local EXIT_STATUS=$?
  set -e
  if [ $EXIT_STATUS -eq 0 ] && [ -f /tmp/report.txt ]; then
    mv /tmp/report.txt "$2"
...
function process_tag() {
  ...
  if [ -f "${OUTPUTDIR}/bin/soljson-${FULL_VERSION_SUFFIX}.js" ]; then
    ...
    if [ -f "scripts/bytecodecompare/storebytecode.sh" ]; then
      echo -ne "GENERATE BYTECODE REPORT FOR ${CYAN}${TAG}${RESET}... "
      generate_bytecode_report "${OUTPUTDIR}/bin/soljson-${FULL_VERSION_SUFFIX}.js" "${OUTPUTDIR}"/log/reports/report-${TAG}.txt
      ...
      if wget -q "$RELEASE_URL/soljson-${HISTORIC_VERSION_SUFFIX}.js" -O /tmp/soljson.js; then
        generate_bytecode_report /tmp/soljson.js "${OUTPUTDIR}"/log/reports/report-historic-${TAG}.txt
      else
    ...
    if [ -f "${OUTPUTDIR}/log/reports/report-${TAG}.txt" ] && [ -f "${OUTPUTDIR}/log/reports/report-historic-${TAG}.txt" ]; then
      ...
      if diff -q "${OUTPUTDIR}/log/reports/report-${TAG}.txt" "${OUTPUTDIR}/log/reports/report-historic-${TAG}.txt" >/dev/null 2>&1; then
        ...
        echo "MATCH" >"${OUTPUTDIR}/log/success/bytecode-${TAG}.txt"
      else
